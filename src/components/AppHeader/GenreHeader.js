...
var GenreHeader = React.createClass({
  ...
  makeOpenGenreHeader(genreCount, colorScale, filteredGenre) {
    ...
    ,   xScale = d3.scaleLinear().domain([0, sum]).range([0, individualWidth])
  ...
  makeClosedGenreHeader(genreCount, colorScale, filteredGenre) {
    ...
    ,   fixedSum = d3.sum(Object.keys(genreCount), (k) => Math.max(genreCount[k], 0.005 * sum))
    ,   xScale = d3.scaleLinear().domain([0, fixedSum]).range([0, this.props.headerWidth])
    ,   cumulative = 0
      ...
      <svg className="GenreHeader" width={this.props.headerWidth} height={48} >
        <g transform={this.props.transform} >
          {this.props.genreList.map((genre) => {
            ...
            ,   p = n / sum
            ,   x = xScale(cumulative)
            cumulative += Math.max(n, 0.005 * sum)
            ...
            return (
              <ProportionalListing
                ...
                width={xScale(Math.max(n, 0.005 * sum))}
                height={20}
                color={colorScale(genre)}
